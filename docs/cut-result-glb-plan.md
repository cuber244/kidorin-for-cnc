...
- Preserve the Kidorin board axes without mirroring: CAM X maps to glTF X, CAM Y maps to glTF Z, and CAM Z maps to glTF Y.
- Put `scale: [0.001, 0.001, 0.001]` on the root node so imported geometry is in meters while metadata remains in millimeters.
- Each `Board_XX` node is the stable work origin at its lower-left top corner. Do not center board geometry around its bounds.
